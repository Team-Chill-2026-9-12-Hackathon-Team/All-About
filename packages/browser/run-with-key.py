...
if len(sys.argv) not in (2, 3):
    raise SystemExit('Usage: python3 run-with-key.py /absolute/path/to/SteelKey.rtf [smoke|smoke:collect|smoke:cancel|smoke:viewer|smoke:timeout]')
script = sys.argv[2] if len(sys.argv) == 3 else 'smoke'
if script not in {'smoke', 'smoke:collect', 'smoke:cancel', 'smoke:viewer', 'smoke:timeout'}:
