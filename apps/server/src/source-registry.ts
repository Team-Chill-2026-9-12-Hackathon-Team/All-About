import { SourceConfigSchema, type SourceConfig } from "@allabout/contracts";

import { demo101Sources } from "./demo101-fixtures.js";

const waterlooScope = { school: "University of Waterloo", campus: "Waterloo", term: null, course: null, section: null, entity: null };
const waterlooPublicSourceRows: Array<[string, string, string, string[]]> = [
  ["waterloo-calendar", "Waterloo Academic Calendar", "https://uwaterloo.ca/academic-calendar/undergraduate-studies/catalog#/home", ["uwaterloo.ca"]],
  ["waterloo-classes", "Waterloo Classes", "https://classes.uwaterloo.ca/under.html", ["classes.uwaterloo.ca"]],
  ["waterloo-events", "Waterloo Events", "https://uwaterloo.ca/events", ["uwaterloo.ca"]],
  ["waterloo-important-dates", "Waterloo Important Dates", "https://uwaterloo.ca/important-dates/undergraduate", ["uwaterloo.ca"]],
  ["waterloo-registrar", "Waterloo Registrar", "https://uwaterloo.ca/registrar/", ["uwaterloo.ca"]],
  ["waterloo-programs", "Waterloo Programs", "https://uwaterloo.ca/future-students/programs", ["uwaterloo.ca"]],
  ["waterloo-policies", "Waterloo Policies", "https://uwaterloo.ca/secretariat/policies-procedures-guidelines", ["uwaterloo.ca"]],
  ["waterloo-student-life", "Waterloo Current Students", "https://uwaterloo.ca/students/", ["uwaterloo.ca"]],
  ["waterloo-recreation-events", "Waterloo Recreation", "https://warrior.uwaterloo.ca/", ["warrior.uwaterloo.ca"]],
  ["waterloo-housing", "Waterloo Campus Housing", "https://uwaterloo.ca/campus-housing/", ["uwaterloo.ca"]],
  ["waterloo-finance", "Waterloo Student Financial Services", "https://uwaterloo.ca/finance/student-financial-services", ["uwaterloo.ca"]],
  ["waterloo-coop", "Waterloo Co-op", "https://uwaterloo.ca/co-operative-education/", ["uwaterloo.ca"]],
  ["waterloo-career", "Waterloo Career Development", "https://uwaterloo.ca/career-development/", ["uwaterloo.ca"]],
  ["reddit-waterloo", "Reddit r/uwaterloo", "https://www.reddit.com/r/uwaterloo/.rss", ["www.reddit.com"]],
  ["uwflow", "UW Flow", "https://uwflow.com/", ["uwflow.com"]],
];
const waterlooSources: SourceConfig[] = waterlooPublicSourceRows.map(([id, label, entryUrl, allowedHosts]) => ({
  id,
  kind: id.startsWith("reddit-") || id === "uwflow" ? "community" : "official",
  label,
  entryUrl,
  allowedHosts,
  scope: waterlooScope,
  contentMode: "live",
  access: "public",
}));

waterlooSources.push({
  id: "waterloo-quest",
  kind: "official",
  label: "Waterloo Quest",
  entryUrl: "https://uwaterloo.ca/the-centre/quest",
  allowedHosts: ["uwaterloo.ca", "quest.pecs.uwaterloo.ca"],
  scope: waterlooScope,
  contentMode: "live",
  access: "authorized",
});

const candidateSources = [
  {
    id: "academic-calendar-csc207",
    kind: "official",
    label: "A&S Academic Calendar — CSC207H1",
    entryUrl: "https://artsci.calendar.utoronto.ca/course/csc207h1",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: "CSC207H1",
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "academic-calendar-csc148",
    kind: "official",
    label: "A&S Academic Calendar — CSC148H1",
    entryUrl: "https://artsci.calendar.utoronto.ca/course/csc148h1",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: "CSC148H1",
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "uoft-events",
    kind: "official",
    label: "University of Toronto Events",
    entryUrl: "https://www.utoronto.ca/events",
    allowedHosts: ["www.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "alumni-carillon-recital",
    kind: "official",
    label: "U of T Alumni — Labour Day Carillon Recital",
    entryUrl: "https://alumni.utoronto.ca/events/labour-day-carillon-recital-0",
    allowedHosts: ["alumni.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: "Labour Day Carillon Recital",
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "soldiers-tower-features",
    kind: "official",
    label: "U of T Alumni — Features of Soldiers' Tower",
    entryUrl: "https://alumni.utoronto.ca/community/soldiers-tower/features-of-soldiers-tower",
    allowedHosts: ["alumni.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: "Labour Day Carillon Recital",
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "academic-calendar-sessional-dates",
    kind: "official",
    label: "A&S Academic Calendar — Sessional Dates",
    entryUrl: "https://artsci.calendar.utoronto.ca/sessional-dates",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "cs-undergrad-courses",
    kind: "official",
    label: "CS Department — Undergraduate courses",
    entryUrl: "https://web.cs.toronto.edu/undergraduate/courses",
    allowedHosts: ["web.cs.toronto.edu"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "academic-calendar-course-search",
    kind: "official",
    label: "A&S Academic Calendar — Course search",
    entryUrl: "https://artsci.calendar.utoronto.ca/search-courses",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: null },
    contentMode: "live",
    access: "public",
  },
  {
    id: "academic-calendar-cs-specialist",
    kind: "official",
    label: "A&S Academic Calendar — Computer Science Specialist",
    entryUrl: "https://artsci.calendar.utoronto.ca/program/asspe1689",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: "Computer Science Specialist" },
    contentMode: "live",
    access: "public",
  },
  {
    id: "academic-calendar-degree-requirements",
    kind: "official",
    label: "A&S Academic Calendar — Degree requirements",
    entryUrl: "https://artsci.calendar.utoronto.ca/degree-requirements-hba-hbsc-bcom",
    allowedHosts: ["artsci.calendar.utoronto.ca"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: null },
    contentMode: "live",
    access: "public",
  },
  {
    id: "cs-program-entry-cmp1",
    kind: "official",
    label: "CS Department — Computer Science admission",
    entryUrl: "https://web.cs.toronto.edu/undergraduate/how-to-apply/cmp1",
    allowedHosts: ["web.cs.toronto.edu"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: "Computer Science Specialist" },
    contentMode: "live",
    access: "public",
  },
  {
    id: "uoft-current-students",
    kind: "official",
    label: "University of Toronto — Current students",
    entryUrl: "https://www.utoronto.ca/current-students",
    allowedHosts: ["www.utoronto.ca"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: null },
    contentMode: "live",
    access: "public",
  },
  {
    id: "uoft-registrar",
    kind: "official",
    label: "University Registrar's Office",
    entryUrl: "https://www.registrar.utoronto.ca/",
    allowedHosts: ["www.registrar.utoronto.ca"],
    scope: { school: "University of Toronto", campus: "UTSG", term: null, course: null, section: null, entity: null },
    contentMode: "live",
    access: "public",
  },
  {
    id: "reddit-uoft-csc207",
    kind: "community",
    label: "Reddit r/UofT",
    entryUrl: "https://www.reddit.com/r/UofT/.rss",
    allowedHosts: ["www.reddit.com"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: "CSC207H1",
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "reddit-uoft",
    kind: "community",
    label: "Reddit r/UofT",
    entryUrl: "https://www.reddit.com/r/UofT/.rss",
    allowedHosts: ["www.reddit.com"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "piazza-login",
    kind: "course_discussion",
    label: "Piazza class discussion",
    entryUrl: "https://piazza.com/login",
    allowedHosts: ["piazza.com"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "authorized",
  },
  {
    id: "artsci-exam-conflicts",
    kind: "official",
    label: "A&S Final Exam Conflicts",
    entryUrl:
      "https://www.artsci.utoronto.ca/current/faculty-registrar/final-exams/exam-conflicts",
    allowedHosts: ["www.artsci.utoronto.ca", "artsci.calendar.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: "final-exams",
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "hart-house-events",
    kind: "official",
    label: "Hart House events",
    entryUrl: "https://harthouse.ca/events/month",
    allowedHosts: ["harthouse.ca", "www.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "student-life-events",
    kind: "official",
    label: "Student Life events",
    entryUrl: "https://www.studentlife.utoronto.ca/events/",
    allowedHosts: ["www.studentlife.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "artsci-academic-dates",
    kind: "official",
    label: "A&S academic dates",
    entryUrl: "https://www.artsci.utoronto.ca/current/dates-deadlines/academic-dates",
    allowedHosts: ["www.artsci.utoronto.ca", "artsci.calendar.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "timetable-builder",
    kind: "official",
    label: "Timetable Builder",
    entryUrl: "https://ttb.utoronto.ca/",
    allowedHosts: ["ttb.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "the-varsity-about",
    kind: "community",
    label: "The Varsity",
    entryUrl: "https://thevarsity.ca/about/",
    allowedHosts: ["thevarsity.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "ulife-organizations",
    kind: "official",
    label: "ULife organizations",
    entryUrl: "https://www.ulife.utoronto.ca/organizations",
    allowedHosts: ["www.ulife.utoronto.ca", "www.studentlife.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "public",
  },
  {
    id: "quercus-login",
    kind: "official",
    label: "Quercus",
    entryUrl: "https://q.utoronto.ca/",
    allowedHosts: ["q.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "authorized",
  },
  {
    id: "acorn-login",
    kind: "official",
    label: "ACORN",
    entryUrl: "https://www.acorn.utoronto.ca/",
    allowedHosts: ["www.acorn.utoronto.ca"],
    scope: {
      school: "University of Toronto",
      campus: "UTSG",
      term: null,
      course: null,
      section: null,
      entity: null,
    },
    contentMode: "live",
    access: "authorized",
  },
  ...waterlooSources,
  ...demo101Sources,
] satisfies SourceConfig[];

export const liveSourceRegistry = SourceConfigSchema.array()
  .min(1)
  .parse(candidateSources);
